...
pfamDesc <- read.delim("../data/pfamDesc.txt", header=F);
colnames(pfamDesc) <- c("PFAM_ID", "PFAM_SD", "PFAM_DESC");
pfamLoc <- read.delim("../data/ucscGenePfam.txt", header=F)
...
pfamDataBioMart <- read.delim("../data/Biomart_PFAM_Export.txt", header=T);
